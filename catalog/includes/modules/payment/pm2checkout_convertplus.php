<?php
require('twocheckout_convertplus/twocheckout_convertplus_library.php');
...
class pm2checkout_convertplus {
	...
	function pm2checkout_convertplus() {
		$this->signature = '2checkout|pm2checkout_convertplus|2.0|2.2';
		...
		if ( (int) MODULE_PAYMENT_2CHECKOUT_CONVERTPLUS_ORDER_STATUS_ID > 0 ) {
			$this->order_status = (int)MODULE_PAYMENT_2CHECKOUT_CONVERTPLUS_ORDER_STATUS_ID;
		}
	 ...
	 */
	function selection() {
		global $cart_pm2checkout_convertplus_id, $order;
	...
	function pre_confirmation_check() {
		...
		$order->info['payment_method'] = '2Checkout Convert Plus';
	}
	...
	function process_button() {
		...
		$process_button_string = tep_draw_hidden_field( 'prod', 'Cart_' . $order_id ) .
		                         tep_draw_hidden_field( 'price', $this->format_raw($order->info['total']) ) .
		                         tep_draw_hidden_field( 'qty', 1 ) .
		                         tep_draw_hidden_field( 'type', 'PRODUCT' ) .
		                         tep_draw_hidden_field( 'tangible', '0' ) .
		                         tep_draw_hidden_field( 'src', 'OSCOMMERCE_'.substr(str_replace('.','_', tep_get_version()),0,3)) .
	...
	function before_process() {
		...
		if (tep_session_is_registered('cart_pm2checkout_convertplus_id') && !empty($_REQUEST['refno'])) {
			$params = $_REQUEST;
			$order_id = substr($cart_pm2checkout_convertplus_id, strpos($cart_pm2checkout_convertplus_id, '-')+1);
			$tco_library = new twocheckout_convertplus_library();

			$new_status = 1;
			$api_response = $tco_library->call( 'orders/' . $params['refno'] . '/', [], 'GET', $this->seller_id, $this->secret_key);
			...
			$order_query = tep_db_query('select orders_id, orders_status from ' . TABLE_ORDERS . ' where orders_id = "' . (int)$order_id . '" limit 1');
			$existing_order = tep_db_fetch_array($order_query);
			if(!empty($existing_order['orders_status'])){
				if($new_status < $existing_order['orders_status']) $new_status = $existing_order['orders_status'];
			}

			tep_db_query("update " . TABLE_ORDERS . " set orders_status = ".(int)$new_status." where orders_id = " . (int)$order_id);
			$sql_data_array = array('orders_id' => (int)$order_id,
			                        'orders_status_id' => (int)$new_status,
			                        'date_added' => 'now()',
			                        'customer_notified' => (SEND_EMAILS == 'true') ? '1' : '0',
			                        'comments' => '2checkout transaction ID: '.$params['refno']);
	 ...
	 */
	function install() {
		$ipn_url =  HTTPS_SERVER . DIR_WS_HTTPS_CATALOG.'ext/modules/payment/twocheckout_convertplus/ipn.php';
		...
		tep_db_query( "insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Seller ID', 'MODULE_PAYMENT_2CHECKOUT_CONVERTPLUS_SELLER_ID', '', 'Seller ID used for the 2CheckOut service', '6', '2', now())" );
		tep_db_query( "insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Secret Key', 'MODULE_PAYMENT_2CHECKOUT_CONVERTPLUS_SECRET_KEY', '', 'Secret key for the 2CheckOut MD5 hash facility', '6', '10', now())" );
		tep_db_query( "insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Secret Word', 'MODULE_PAYMENT_2CHECKOUT_CONVERTPLUS_SECRET_WORD', '', 'Secret word for the 2CheckOut MD5 hash facility', '6', '10', now())" );
		...
		tep_db_query( "insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('IPN_URL', 'MODULE_PAYMENT_2CHECKOUT_CONVERTPLUS_IPN_URL','$ipn_url', 'The callback endpoint for IPN requests from 2Checkout', '6', '2', 'tco_cplus_edit_readonly_text( ', now())");
	...
	function keys() {
		return array(
			...
			'MODULE_PAYMENT_2CHECKOUT_CONVERTPLUS_IPN_URL',
...
function tco_cplus_edit_readonly_text($text, $key= ''){
	return tco_cplus_tep_draw_input_field($key, $text);
}
 ...
 * @return string
 */
function tco_cplus_tep_draw_input_field($name, $value = '', $parameters = '', $required = false, $type = 'text', $reinsert_value = true) {
